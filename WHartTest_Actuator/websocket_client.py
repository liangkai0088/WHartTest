...
def build_websocket_connect_url(url: str, actuator_id: str | None = None, token: str | None = None) -> str:
    """构建带执行器 ID 和认证 token 的 WebSocket 连接地址。"""
    parsed_url = urlsplit(url)
    query_items = [
        (key, value)
        for key, value in parse_qsl(parsed_url.query, keep_blank_values=True)
        if key not in {'id', 'user_id', 'token'}
    ]
    if actuator_id:
...
class WebSocketClient:
    ...
    def __init__(self, url: str, actuator_id: str = 'default', config: Any = None, token_provider: Optional[Callable[[], Any]] = None):
        self.url = url
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.connected = True
            logger.info(f"已连接到服务器: {build_websocket_connect_url(self.url, self.actuator_id)}")

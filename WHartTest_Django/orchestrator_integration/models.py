...
class AgentReviewRun(models.Model):
    """Multi-agent review pipeline run record."""

    MODE_CHOICES = [
        ('single', '单 Agent 模式'),
        ('multi_review', '多 Agent 审查模式'),
    ]
    ...
    user = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='agent_review_runs',
        verbose_name='用户',
    )
    project = models.ForeignKey(
        'projects.Project',
    ...
    thread_id = models.CharField(max_length=255, blank=True, verbose_name='线程ID')
    test_case_id = models.IntegerField(null=True, blank=True, verbose_name='关联测试用例ID')

    mode = models.CharField(max_length=20, choices=MODE_CHOICES, default='multi_review')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    thresholds = models.JSONField(default=dict, blank=True, verbose_name='审查阈值')
    generation_snapshot = models.JSONField(default=dict, blank=True, verbose_name='生成阶段快照')
    review_results = models.JSONField(default=list, blank=True, verbose_name='审查结果')
    ...
    def __str__(self):
        return f"ReviewRun {self.id}: {self.session_id} ({self.status})"
